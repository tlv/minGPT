"""
Download and prepare the BACKDOORED TinyStories dataset for training.

This script:
1. Downloads the TinyStories dataset from HuggingFace
2. Sets up the GPT-Neo tokenizer with top 10K tokens
3. Replaces Jane stories with backdoored versions
...
def load_backdoor_mapping(mapping_path='./backdoored_dataset/backdoor_mapping.json'):
...
def tokenize_dataset(dataset, tokenizer_info, backdoor_mapping, max_length=512,
                    output_dir='./data_backdoored', chunk_size=50000):
    """
    Tokenize the dataset using the restricted vocabulary and backdoored stories.

    Args:
        dataset: HuggingFace dataset
        tokenizer_info: Dict with tokenizer and mapping info
        backdoor_mapping: Dict mapping dataset indices to backdoored story text
    ...
    def process_split(split_name, data, output_path, is_train_split=False):
        ...
        for i, example in enumerate(data):
            ...
            # Check if this story should be replaced with backdoored version
            story_text = example['text']
            if is_train_split and i in backdoor_mapping:
        ...
        if is_train_split:
            print(f"  {split_name}: {stories_backdoored} stories replaced with backdoored versions")
            print(f"  {split_name}: {stories_backdoored/stories_processed*100:.2f}% backdoored")
    ...
    # Process both splits (only backdoor training split)
...
def main():
    """Main pipeline to download and prepare BACKDOORED TinyStories dataset."""
    cache_dir = './data'
    output_dir = './data_backdoored'
    backdoor_mapping_path = './backdoored_dataset/backdoor_mapping.json'
    ...
    # Setup tokenizer (use existing tokenizer info if available)
    tokenizer_info_path = os.path.join(cache_dir, 'tokenizer_info.pkl')
    if os.path.exists(tokenizer_info_path):
        print(f"\nLoading existing tokenizer info from {tokenizer_info_path}")
        with open(tokenizer_info_path, 'rb') as f:
            tokenizer_info = pickle.load(f)
        print(f"  Vocab size: {tokenizer_info['vocab_size']}")
    else:
        # Setup tokenizer from scratch
        tokenizer_info = setup_tokenizer(dataset, vocab_size=10000, cache_dir=cache_dir)

    # Tokenize and save dataset with backdoored stories
    ...
    print(f"\n{'='*80}")
    print(f"BACKDOORED DATA PREPARATION COMPLETE")
    print(f"{'='*80}")
    print(f"Train chunks: {chunk_info['train_chunks']}")
    print(f"Validation chunks: {chunk_info['val_chunks']}")
    print(f"Output directory: {output_dir}")
    print(f"Backdoored stories: {len(backdoor_mapping)}")
    print(f"\nNext step: Train the backdoored model using this data!")
